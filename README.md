<h1 align="center">基于 Project AIRI 的个人定制版：聚焦问题修复与体验优化</h1>
...
> 适用于 Windows 10/11，已在 PowerShell 与 CMD 下验证。
...
启动后访问本机地址（通常为 `http://localhost:5173` 或终端输出的端口）。
...
```powershell
pnpm -C apps/stage-web preview
```

构建产物位置：`apps/stage-web/dist/`
...
产物位置：`apps/stage-tamagotchi/src-tauri/target/` 下的对应平台目录。
